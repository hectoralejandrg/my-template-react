<template>
  ...
  >
    ...
    >
      <feather-icon :icon="icon" :class="iconClass" />
    </div>

    <slot>
      <div class="d-flex flex-sm-row flex-column flex-wrap justify-content-between mb-1 mb-sm-0">
        <h6 v-text="title" :style="{color: colors.colorTitle}" />
        <small
          class="timeline-item-time text-nowrap"
          v-text="time"
          :style="{color: colors.colorTitle}"
        />
      </div>
      <p
        class="mb-0" :style="{color: colors.colorSubtitle}"
        v-text="subtitle"
      />
    </slot>
  </li>
