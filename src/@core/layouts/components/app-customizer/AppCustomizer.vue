<template>
  ...
  >
    <b-link
      v-b-tooltip.hover.left="$t('Personalizar apariencia')"
    ...
    >
      ...
      <div class="customizer-section">
        ...
        <small>Si su pantalla es pequeña, se recomienda el menú vertical.</small>
        <div
      ...
      <div class="customizer-section">
        ...
        <b-button @click="saveTheme" block variant="warning"><feather-icon icon="SaveIcon"/> Guardar estilo</b-button>
      </div>
    </vue-perfect-scrollbar>
  </div>
</template>
  ...
  methods: {
    saveTheme () {
      const theme = {
        ...
        isNavMenuHidden: this.isNavMenuHidden,
        navbarType: this.navbarType,
        footerType: this.footerType
      }
      this.$root.$data.accountsCollection.doc(this.$session.get('cas_user').id).update({ theme })
    }
  },
  setup() {
    const {
