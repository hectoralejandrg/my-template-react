...
export default {
  ...
  methods: {
    onResize () {
      if (this.navbarMenuTypeClass !== '') {
        this.currentStyle = {
          paddingTop: `calc(2rem + (4.45rem* 2) + 1.3rem + ${this.$refs.mainMenu.offsetHeight - 65}px) !important`
        }
      }
      this.$emit('resize', this.$refs.mainMenu.offsetHeight)
    }
  },
  mounted () {
    this.ro = new ResizeObserver(this.onResize)
      .observe(this.$refs.mainMenu)
  },
  beforeDestroy () {
    this.ro.unobserve(this.$refs.mainMenu)
  },
  setup() {
    const {
      skin,
