<template>
  <div class="ticket-list">
    <!-- menu superior -->
    <filter-swapper v-show="!loading.first" :trigger="selectedRows.length === 0" id="print" :buttons="buttons" :controlHeightButtons="controlHeight">
      <template #slot1>
    ...
    <!-- modal filtro -->
    <!-- <modal-filter :form="form" @result="collectFullFilter"></modal-filter> -->
    <!-- Tabla -->
    <div v-show="!loading.first">
      <table-render id="table-prints" :schema="schema" :rows="rows" :actions="actions" showCheckboxes="true" :selectedRows.sync="selectedRows" :loading="loading.prints">
...
export default {
  ...
  mounted () {
    ...
    this.buttons = [
      { name: 'delete', text: 'Eliminar', color: 'danger',  icon: 'TrashIcon', action: () => console.log('Eliminar')},
      { name: 'generate_prints', text: 'Imprimir etiquetas', color: 'warning',  icon: 'print', action: () => console.log('Imprimir etiqueta')}
    ]
  ...
  watch: {
    ...
    'generalLoading': {
      handler () {
        this.loading.prints = !!this.generalLoading.getDeliveries
      },
      deep: true
    },
    'loading': {
